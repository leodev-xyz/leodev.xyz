...
on_player_hurt = ->
    if Entity.GetEntityFromUserID(Event.GetInt "attacker") == Entity.GetLocalPlayer() && shots.length > 0
        this_tick_shot = 2
        print "[\x04RAGEBOT\x01] Hit \x03" + Entity.GetName(Entity.GetEntityFromUserID Event.GetInt "userid") + "\x01 for \x0b" + Event.GetInt("dmg_health") + "\x01 damage in \x0b" + HITBOX[Event.GetInt "hitgroup"] + "\x01\n", [0, 255, 0, 255]
